...
def extract_date_from_row(element):
    """Fallback to find any month/day text if the main date container is empty."""
    # List of common month patterns
    ...
    # Try to find just "Month"
    match_only_month = re.search(month_pattern, text, re.IGNORECASE)
    if match_only_month:
        return match_only_month.group(1)
    ...
    try:
        ...
        if "goheels.com" in url:
            ...
            for item in soup.select('.c-event-card'):
                ...
                if opp_tag:
                    games.append({
                        "Date": date_tag.get_text(strip=True) if date_tag else extract_date_from_row(item),
            ...
            for item in soup.select('.sidearm-schedule-game'):
                ...
                if opp_el:
                    ...
                    date_val = "TBD"
                    if date_container:
                        # Extract nested spans (usually Month and Day are separate)
                        spans = date_container.find_all('span')
                        if spans:
                            date_val = " ".join([s.get_text(strip=True) for s in spans if s.get_text(strip=True)])
                        else:
                            date_val = date_container.get_text(" ", strip=True)
                    
                    # If it's still TBD or just says 2026, search the whole row
                    if date_val == "TBD" or date_val.strip() == "2026":
                        date_val = extract_date_from_row(item)

                    games.append({"Date": date_val.replace("2026", "").strip(), "Opponent": clean_opp, "Status": res_el.get_text(strip=True) if res_el else "Scheduled", "Venue": venue})
